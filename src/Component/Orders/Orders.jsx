...
const Orders = () => {
  ...
  const acceptOrder = () => {
    axios
      .put(`http://localhost:8000/api/orders/active-order/${orderId}/1`)
      .then(res => {
        console.log(res)
        fetchData()
        setIsShowAcceptModal(false)
  ...
  const acceptRejectOrder = () => {
    axios
      .put(`http://localhost:8000/api/orders/active-order/${orderId}/0`)
      .then(res => {
        console.log(res)
        fetchData()
        setIsShowRejectModal(false)
  ...
  return (
    <div className='mt-10 p-5'>
      <h1 className='mb-5 text-xl'>سفارش ها</h1>
      {allData.length ? (
