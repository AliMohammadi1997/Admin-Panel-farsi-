...
const Sidebar = ({ toggleBtn,isActive }) => {
    return (
        <div className={`fixed  lg:sticky bg-blue1 h-screen w-32 md:w-64   top-0 right-0 p-4 transform transition-transform ${isActive ? 'translate-x-0' : 'translate-x-full'} lg:translate-x-0`}>
            <button className="text-lg mb-6 fixed top-3 left-3 lg:hidden" onClick={toggleBtn}>
                <IoClose  />
            </button>
            <h1 className="hidden lg:inline-block text-xl text-white  p-4  text-center border-b-2 border-solid  border-blue2 w-full">به داشبورد خود خوش آمدید</h1>
